...
// swiftSuitePath resolves the Swift honest measurement corpus relative to the
// repo root. The CLI package lives in ./cli, so the corpus is one directory up.
func swiftSuitePath() string {
	return filepath.Join("..", "testdata", "precision-suite-swift")
}

// TestPrecisionSuiteBaselineSwift is the ratchet for the Swift corpus: it scans
...
// deliberately scores below 1.0 on recall — it measures nox's Swift taint model
// against ground truth so real recall gaps (the documented property-assignment
// false negative) show up as a number. Pinning that number here means Swift
...
func TestPrecisionSuiteBaselineSwift(t *testing.T) {
	dir := swiftSuitePath()
	...
	if len(expectations) == 0 {
		t.Fatal("swift suite has no expectations; a labeled corpus must declare some")
	}
	...
	if regressions := bench.CompareBaseline(base, current); len(regressions) > 0 {
		for _, r := range regressions {
			t.Errorf("swift suite regressed: %s", r.String())
		}
		t.Fatalf("swift precision suite regressed vs baseline.json; investigate the change or, if intended, "+
			"regenerate the snapshot with `nox bench --precision testdata/precision-suite-swift "+
			"--baseline testdata/precision-suite-swift/baseline.json` after deleting it.\n%s",
			renderPrecisionTable(dir, &report))
	}

	if bench.Improved(base, current) {
		t.Logf("swift precision suite IMPROVED vs baseline.json (precision %.3f->%.3f, recall %.3f->%.3f, FP %d->%d); "+
			"refresh testdata/precision-suite-swift/baseline.json to lock in the gain",
			base.Precision, current.Precision, base.Recall, current.Recall, base.FP, current.FP)
	}

	// Precision floor: the Swift model must stay at or above the 0.90 gate CI
	// enforces via --min-precision, independent of the baseline ratchet.
	if p := report.Overall.Precision(); p < 0.90 {
		t.Errorf("swift suite precision %.3f is below the 0.90 floor:\n%s",
